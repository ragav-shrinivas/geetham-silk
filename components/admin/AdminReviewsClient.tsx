      ...
      <div className="flex gap-2 mb-6">
        {(['pending', 'approved', 'all'] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3.5 py-1.5 text-xs tracking-wide uppercase rounded-full border transition-colors ${filter === f ? 'bg-[var(--brand-darkpink)] text-white border-[var(--brand-charcoal)]' : 'bg-white text-gray-500 border-gray-200 hover:border-[var(--brand-rose)]'}`}>
